    ...
    create() {
        ...
        let pauseButton = this.add.text(this.game.config.width*0.03, this.game.config.height*0.02, "⏸️")
            ...
            .on('pointerdown', () => {
                ...
                this.scene.launch('pause', {bgm: this.bgm, pB: pauseButton, currLevel: 'level3'});
        ...
        let enemy2Counter = 0;
        enemy2.createMultiple({key: this.playersprite[2], quantity: 4})
        ...
        let enemy3 = this.add.existing(new Enemies(this.physics.world, this, {name: 'enemy3'}));
        let enemy3Counter = 0;
        enemy3.createMultiple({key: this.playersprite[3], quantity: 2})
        let enemySpawn3 = this.time.addEvent({delay: 800, loop: true, callback: () => {
            if(enemy3.spawn(Phaser.Math.RND.between(200, 900), -200, 0, 800, this.playersprite[4])) {enemy3Counter++;}
            if(enemy3Counter == 12) {
                enemySpawn3.remove();
                //console.log('wave cleared');
            }
        ...
        this.physics.add.overlap(this.playerBullets, enemy3, (bullet, enemy) => {
            this.kills.push(3);
            this.killCount++;
